# Гороскоп Telegram Bot (v0.3 PROD RUN)

Telegram-бот на aiogram 3.x для модуля «Гороскоп» с рабочей связкой Telegram ↔ OpenAI и расширенной диагностикой запуска.

## Быстрый старт
1. Запустите `START_LAUNCHER.bat` (Windows, требует `py -3.12`). Скрипт создаст `.venv`, поставит зависимости и откроет GUI.
2. Если нужно вручную: создайте файл `.env` по примеру `.env.example` и укажите `BOT_TOKEN`, затем `pip install -r requirements.txt`.
3. GUI-лаунчер: `python launch.py`.
4. CLI без GUI: `python launch.py --run-bot`.
5. Проверка OpenAI: в GUI (Настройки → «Проверить OpenAI») или CLI: `python launch.py --test-ai`.

## Возможности
- Главное меню с разделом «Гороскоп», балансом, настройками и информацией.
- Пошаговый сбор данных для прогнозов «На сегодня» и «На неделю» через FSM.
- Проверка и списание бесплатных лимитов (3 запроса по умолчанию).
- Рабочая связка с OpenAI (модель `gpt-4o-mini`). При `USE_OPENAI=true` без ключа бот автоматически уходит в Stub-режим и сообщает об этом в логах/UI.
- Подготовленные точки для интеграции оплаты (PaymentService interface).
- GUI-лаунчер (Tkinter) для настройки .env, теста AI, симуляции сценария и управления ботом.

## Launch.py и GUI
- `python launch.py` — открывает графический лаунчер «GOROSKOPE Launcher» без необходимости BOT_TOKEN.
- `python launch.py --print-env` — выводит текущие настройки с маскировкой токенов.
- `python launch.py --test-ai` — консольный прогон промпта с активным AI-режимом (OpenAI или stub).
- `python launch.py --run-bot` — запускает бота напрямую (аналог `python bot.py`).

### Табы GUI
- **Настройки** — редактирование и валидация `.env` (BOT_TOKEN, USE_OPENAI, OPENAI_API_KEY, LOG_LEVEL, DB_PATH) + кнопка «Проверить OpenAI».
- **Тест AI** — генерация промпта и вызов текущего AI (stub/OpenAI) без Telegram.
- **Симулятор** — чат-эмулятор сценария «Гороскоп» с валидацией данных и списанием квоты в SQLite.
- **Запуск** — запуск/остановка `bot.py` в подпроцессе, просмотр логов.
- **Мини-редактор** — сохранение поверхностных правок в `bot_overrides.json` (FREE_QUOTA, тексты, стиль промпта).

## AI и режимы работы
- **OpenAI**: установите `USE_OPENAI=true` и задайте `OPENAI_API_KEY`. Используется модель `gpt-4o-mini`, async-клиент с таймаутами и повторными попытками. Ошибки (429/401/5xx) логируются и возвращают пользователю безопасное сообщение.
- **Stub**: если `USE_OPENAI=false` или ключ не указан, включается StubAIService. Пользователь видит пометку о демо-режиме, в логах фиксируется режим.
- **Логи**: пишутся в `logs/app.log` и консоль (лаунчер — `logs/launcher.log`). Глобальный error handler логирует трейс и отправляет пользователю безопасное уведомление.

## Overrides
- Пользовательские правки сохраняются в `bot_overrides.json` (значения квоты, стоимости, тексты, стиль промпта).
- `settings.py` подхватывает `OVERRIDES_PATH`, `FREE_QUOTA`, `REQUEST_PRICE_STARS`; `runtime_config` применяет JSON поверх .env.
- Тексты (`app/core/texts.py`) и `PromptBuilder` умеют читать overrides без изменения основной архитектуры.

## Стек
- Python 3.11+
- aiogram 3.x
- aiosqlite
- pydantic-settings
